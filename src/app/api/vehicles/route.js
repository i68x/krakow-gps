...
function getBlocks() {
  if (!blocksCache) {
    try {
      blocksCache = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'public', 'data', 'blocks_all.json'), 'utf8'));
    } catch { blocksCache = {}; }
  }
  return blocksCache;
}

let mRouteMapCache = null;
function getMRouteMap() {
  if (!mRouteMapCache) {
    try {
      mRouteMapCache = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'public', 'data', 'm_route_map.json'), 'utf8'));
    } catch { mRouteMapCache = {}; }
  }
  return mRouteMapCache;
}
  ...
  try {
    ...
    vResults.forEach(({ type, entities }) => {
      entities.forEach(e => {
        ...
        const rtRouteId = v.trip?.routeId || '';
        const tripInfo = trips[type]?.[tid];
        // Better route resolution: use trip data, fallback to RT routeId (M), then routeId raw
        let routeName = tripInfo?.[0] || '';
        if (!routeName && type === 'M' && rtRouteId) {
          routeName = mRouteMap[rtRouteId] || rtRouteId;
        }
        if (!routeName) routeName = rtRouteId || '?';
        ...
        // Brigade for ALL types using blocks_all
        const brigade = blocks[`${type}:${blockId}`] || blockId || '';
